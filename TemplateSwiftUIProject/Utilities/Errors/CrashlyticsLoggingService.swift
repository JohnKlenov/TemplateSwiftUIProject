...
///Я хочу создать сервис для логирования ошибок в Crashlytics Firebase ! В моем приложении у меня есть различные сущности(ErrorHandlers) которые принимают ошибку, определяют ее домен и возвращают локализированный текст этой ошибки для алерта или же просто в нутри своей логики проводят онализ и при необходимости логируют сообщения для команды разработчиков на Crashlytics Firebase. Дак вот для таких сущностей как ErrorHandlers я хочу создать сервис для логирования ошибок в Crashlytics Firebase и использовать ее в ErrorHandlers как зависимость для передачи logToCrashlytics.
