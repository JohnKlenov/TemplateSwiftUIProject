...
//
//  ROADMAP: Обновление macOS, Xcode и настройка проекта под iOS 18 → iOS 26
//
//  1. ОБНОВЛЕНИЕ macOS
//  ------------------------------------------------------------
//  • Текущая версия: macOS 14.4 — НЕ поддерживает Xcode 26.x
//  • Требование: macOS 15.6 или новее
//  • Действия:
//      - Открыть System Settings → General → Software Update
//      - Обновиться до macOS 15.6+
//      - Перезагрузить Mac
//
//  2. УСТАНОВКА Xcode С ПОДДЕРЖКОЙ iOS 26 SDK
//  ------------------------------------------------------------
//  • После обновления macOS зайти в App Store
//  • Скачать Xcode 26.x (например, 26.3)
//  • В составе Xcode 26.x идут SDK:
//      - iOS 26.2
//      - iPadOS 26.2
//      - watchOS 26.2
//      - tvOS 26.2
//      - visionOS 26.2
//      - macOS 26.2
//  • После установки Xcode автоматически добавит новые SDK
//
//  3. НАСТРОЙКА ПРОЕКТА ДЛЯ ПОДДЕРЖКИ iOS 18 → iOS 26
//  ------------------------------------------------------------
//  • Открыть Xcode → Project → TARGETS → TemplateSwiftUIProject
//  • Перейти в раздел "Info"
//  • Установить:
//        iOS Deployment Target = iOS 18.0
//  • Перейти в раздел "General"
//  • Убедиться, что в списке Minimum Deployments появилась iOS 18
//  • Теперь приложение будет работать на iOS 18 → iOS 26
//
//  4. КАК УБЕДИТЬСЯ, ЧТО КОД ПОДДЕРЖИВАЕТ iOS 18 И iOS 26
//  ------------------------------------------------------------
//  ✔ Deployment Target = iOS 18 — фундамент совместимости
//  ✔ Xcode автоматически проверяет API-доступность:
//        - Если API доступен только с iOS 26, Xcode покажет ошибку
//        - Ты физически не сможешь собрать несовместимый код
//  ✔ Проверить сторонние библиотеки (Firebase, SDWebImage):
//        - Они поддерживают iOS 18, проблем не будет
//  ✔ Запустить приложение на симуляторах:
//        - iOS 18
//        - iOS 26
//  ✔ Проверить ключевые экраны SwiftUI:
//        - Навигация
//        - Анимации
//        - WebImageView
//        - PhotosPicker (если используется)
//  ✔ Загрузить сборку в App Store Connect:
//        - Если что-то несовместимо, Apple не пропустит
//
//  5. ИТОГ НАШЕГО СЕГОДНЯШНЕГО РАЗГОВОРА
//  ------------------------------------------------------------
//  • Apple перепрыгнула с iOS 18 сразу на iOS 26 (версий 19–25 не существует)
//  • Чтобы поддерживать iOS 18 и iOS 26, нужен Xcode 26.x
//  • Чтобы установить Xcode 26.x, нужен macOS 15.6+
// –  • После обновления macOS и Xcode появятся SDK для iOS 18 и iOS 26
//  • Deployment Target = iOS 18 — лучший выбор для старта приложения
//  • Xcode сам гарантирует, что код не использует API новее iOS 18
//  • Тестирование на симуляторах iOS 18 и iOS 26 подтверждает совместимость
//
//  6. КОРОТКИЙ ЧЕК-ЛИСТ
//  ------------------------------------------------------------
//  ☐ Обновить macOS до 15.6+
//  ☐ Установить Xcode 26.x
//  ☐ Установить Deployment Target = iOS 18
//  ☐ Проверить API-доступность (Xcode сам покажет ошибки)
//  ☐ Проверить библиотеки (Firebase/SDWebImage — ок)
//  ☐ Запустить на симуляторах iOS 18 и iOS 26
//  ☐ Загрузить сборку в App Store Connect
//
//  После выполнения всех шагов проект гарантированно поддерживает iOS 18 → iOS 26.
//
//
