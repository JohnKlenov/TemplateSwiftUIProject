...
class ContentAccountViewModel: ObservableObject {
    ...
    func retryUserProfile() {
        switch authorizationManager.validateUserForProfileLoading() {
        case .success(let uid):
            userInfoCellManager.loadUserProfile(uid: uid)
        case .failure:
            // UI ничего не делает — ошибка уже залогирована
            return
        }
    }
...
// MARK: - before authorizationManager.validateUserForProfileLoading()
