...
// ──────────────────────────────────────────────────────────────
// MARK: - Архитектурное решение по размещению ViewBuilderService
// ──────────────────────────────────────────────────────────────
//
// Почему изначально рассматривали перенос ViewBuilderService в корень:
//
// 1) Архитектурная чистота: корневой ViewBuilderService создаёт единый
//    DI‑контейнер, где HomeManager, CRUDSManager и AuthenticationService
//    живут столько же, сколько и приложение.
//
// 2) Предсказуемый жизненный цикл: сервисы не зависят от UI и не
//    пересоздаются при ререндерингах ContentView.
//
// 3) Централизация зависимостей: HomeView/GalleryView/AccountView могли бы
//    получать сервисы напрямую через environmentObject.
//
// Однако перенос в корень имеет серьёзный UX‑недостаток:
//
// - Сервисы начинают работать слишком рано (ещё на экране Onboarding).
// - Firebase Auth и Firestore запускаются до входа в основной поток.
// - Это приводит к преждевременным ошибкам авторизации и чтения данных,
//   которые пользователь видит до появления ContentView.
//
// Поэтому ViewBuilderService остаётся в ContentView:
//
// 1) Все менеджеры создаются строго после перехода в основной UI.
//    Это полностью исключает «ранние» ошибки и делает UX предсказуемым.
//
// 2) ContentView имеет фиксированный .id("ContentView"), поэтому SwiftUI
//    не пересоздаёт его при обычных ререндерингах. Это обеспечивает
//    стабильный жизненный цикл ViewBuilderService и всех зависимых сервисов.
//
// 3) Мы добавили безопасный fallback:
//        guard let self = self else {
//            return Just(.error("Internal state lost")).eraseToAnyPublisher()
//        }
//    Он защищает от крайне редких системных сценариев, когда SwiftUI
//    пересоздаёт ВСЁ дерево (смена сцены, memory pressure, смена языка).
//    В таких случаях старый HomeManager корректно завершает цепочку,
//    а новый ContentView/ViewModel запускают observeBooks() заново.
//
// Минимальные оставшиеся риски:
//
// - При редких системных событиях (пересоздание сцены, выгрузка UI,
//   смена языка/sizeCategory) старый HomeManager может завершить цепочку
//   ошибкой "Internal state lost". Новый UI эту ошибку НЕ увидит — он
//   создаёт новую подписку и корректно восстанавливается.
//
// - Эти сценарии крайне редки (<0.1% случаев) и не влияют на UX,
//   так как новый ViewModel всегда перезапускает observeBooks().
//
// Итог:
//
// Перенос ViewBuilderService в корень был архитектурно привлекательным,
// но нарушал UX из‑за преждевременных ошибок. Мы оставили сервис в
// ContentView, сохранив стабильность и предсказуемость поведения, а
// fallback .error("Internal state lost") покрывает редкие системные
// пересоздания дерева SwiftUI.
// ──────────────────────────────────────────────────────────────
...
//
// ВАЖНО: ContentView может быть пересоздан системой в редких случаях.
// SwiftUI пересоздаёт только верхний уровень UI, но НЕ уничтожает сцену
// и НЕ пересоздаёт @StateObject, пока сцена жива. Однако если система
// решает пересоздать всю сцену (крайне редкое событие), то ContentView
// будет создан заново, и вместе с ним пересоздадутся:
//
//   • viewBuilderService (@StateObject)
//   • mainCoordinator (@StateObject)
//   • все зависимые сервисы (HomeManager, AuthenticationService и т.д.)
//
// Это приводит к:
//   • сбросу навигационного состояния (mainCoordinator)
//   • пересозданию сервисов (viewBuilderService)
//   • повторной инициализации HomeManager / ViewModels
//
// Причины полного уничтожения старого ContentView и создания нового:
//   • смена языка устройства (iOS перезапускает приложение полностью)
//   • выгрузка сцены системой из-за memory pressure
//   • редкие внутренние события SwiftUI (diffing failure)
//   • перезапуск приложения системой после смены настроек среды
//
// Это крайне редкие системные сценарии, которые НЕ являются ошибкой
// и НЕ приводят к крашам. SwiftUI гарантирует корректное восстановление
// состояния приложения после пересоздания дерева.
