...
// Средний блок
// Средний блок горизонтальная карусель c item (Droplist || All tracks for Droplist || GYM || Party || R&B ... )(как в VK на сообществах) При прокрутки общего скрола композитного View (Верхний блок + Средний блок +  Нижний блок) вниз, Средний блок - блок горизонтальной карусели прилипает к верху экрана (navigation bar) и далее идет лента с плэйлистами.
// при выборе item из блока горизонтальной карусели подгружаются данные в Нижний блок - вертикальная лента.
...
// Средний блок
// получаем данные для заполнения названия item то есть текст + ссылка для получения данных по нажатию на item.
// при первом запуске экрана получаем данные для первого item Droplist (но так как это довольно большой список хотелось бы иметь подгрузку данных порциями - пагимнацию)
